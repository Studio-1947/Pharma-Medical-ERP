  ...
  private buildPdf(invoice: any, branch: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(8).font("Helvetica")
        ...
        .text([branch?.phone, branch?.email].filter(Boolean).join("  |  "), ML + 12, 74, { width: PW * 0.55 });
      doc.fontSize(16).font("Helvetica-Bold").fillColor("white")
        .text("TAX INVOICE", ML + PW * 0.58, 42, { width: PW * 0.42, align: "right" });
      doc.fillColor(BLACK);
      ...
      doc.fontSize(7.5).font("Helvetica-Bold").fillColor(GRAY).text("BILLED TO", col1X, metaY + 8);
      doc.fontSize(9).font("Helvetica-Bold").fillColor(BLACK)
        .text(invoice.patient?.name ?? "Walk-in Customer", col1X, metaY + 18);
      if (invoice.patient?.phone) {
        doc.fontSize(8).font("Helvetica").fillColor(GRAY).text(`Ph: ${invoice.patient.phone}`, col1X, metaY + 29);
      }

      const fieldLabel = (label: string, value: string, y: number) => {
        doc.font("Helvetica-Bold").fontSize(7.5).fillColor(GRAY).text(label, col2X, y, { width: 80 });
        doc.font("Helvetica").fontSize(8.5).fillColor(BLACK).text(value, col2X + 82, y, { width: PW * 0.42 });
      };
      ...
      headers.forEach((h, i) => {
        doc.font("Helvetica-Bold").fontSize(7.5).fillColor(GRAY)
          .text(h, cx + 4, tableY + 5, { width: colWidths[i]! - 6, align: i >= 2 ? "right" : "left" });
        cx += colWidths[i]!;
      });
      ...
      items.forEach((item: any, idx: number) => {
        ...
        rowCols.forEach((col, i) => {
          doc.font("Helvetica").fontSize(7.5).fillColor(BLACK)
            .text(col.text, cx + 4, rowY + 4, { width: colWidths[i]! - 6, align: col.align as any });
          cx += colWidths[i]!;
        });
        rowY += rowH;
      });
      ...
      const totLine = (label: string, value: string, bold = false, color = BLACK) => {
        doc.font(bold ? "Helvetica-Bold" : "Helvetica").fontSize(bold ? 9 : 8).fillColor(GRAY)
          .text(label, totLineX, rowY, { width: PW * 0.22 });
      ...
      if (invoice.payments?.length) {
        doc.font("Helvetica-Bold").fontSize(8).fillColor(GRAY).text("Payment Details:", ML, rowY);
        rowY += 12;
      ...
      doc.fontSize(7).font("Helvetica").fillColor(GRAY)
        .text("This is a computer-generated invoice. No signature required.", ML, footerY + 8, { width: PW * 0.6 });
      doc.text(`Generated on ${new Date().toLocaleString("en-IN")}`, ML + PW * 0.6, footerY + 8, { width: PW * 0.4, align: "right" });
      if (invoice.prescriptionId) {
        doc.fontSize(7.5).font("Helvetica-Bold").fillColor("#7c3aed")
          .text("Rx — Dispensed against verified prescription", ML, footerY + 20, { width: PW });
      }

  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <input
        ...
        className={
          className ??
          `w-full border rounded-xl px-3 py-1.5 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500 transition-colors ${
            linked ? "bg-emerald-50/60 border-emerald-300 text-emerald-950 font-bold" : "bg-white text-slate-900 border-slate-200"
          }`
        }
      />
      ...
      {showDropdown && (
        <div
          ref={listRef}
          role="listbox"
          className="absolute z-50 left-0 w-80 sm:w-[360px] mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-60 overflow-y-auto p-1 ring-1 ring-black/5"
        >
            ...
            results.map((m, idx) => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 transition-all rounded-lg border-b last:border-b-0 border-slate-100 ${
                  idx === highlight ? "bg-emerald-50 border-emerald-200" : "hover:bg-slate-50"
                }`}
              >

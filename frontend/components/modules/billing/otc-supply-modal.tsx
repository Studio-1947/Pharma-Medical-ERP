 ...
 *
 * A walk-in rarely buys one thing, so the sale carries a list of medicines,
 * not one. The medicine that opened the modal is the first line; the search
 * below the list adds more, and the whole list goes out as a single invoice —
 * one bill, one payment, one invoice number, the way the counter hands it over.
 */
...
export function OtcSupplyModal({
  ...
  // The parent keeps this component mounted between sales, so without this the
  ...
  // ── Add another medicine to the same bill ─────────────────────────────────
  ...
  const { data: searchRaw, isFetching: searching } = useQuery({
    ...
    queryFn: () =>
      apiClient.get("/inventory/medicines", {
        params: { search: debouncedSearch.trim(), limit: 6 },
      }) as any,
  ...
  const subtitle =
    lines.length > 1
      ? `${lines.length} medicines on this bill`
      : `${medicine?.name ?? "Medicine"}${medicine?.sku ? ` · ${medicine.sku}` : ""}`;
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full text-sm border border-slate-200 rounded-lg pl-9 pr-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/30"
                />
              </div>
              {searchActive && (
                <div className="mt-2 rounded-lg border border-slate-200 divide-y divide-slate-100 overflow-hidden">
                  ...
                  {searchResults.map((m: any) => (
                    <button
                      ...
                      className="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-left hover:bg-orange-50/60 transition-colors"
                    >
                      ...
                      <span className="flex items-center gap-2 shrink-0">
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-orange-500 text-white text-[11px] font-bold">
                          <Plus size={11} /> Add
                        </span>
                      </span>
                    </button>
